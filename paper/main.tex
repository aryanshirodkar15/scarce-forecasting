...
\normalsize \textit{Affiliation to be completed}\\
...
730-day frame, regardless of how much history the cell subsequently hands to the
...
\gap{The cross-validation split strategy is not yet fixed. The open tension is
that rolling-origin folds on a 60-day window are necessarily few and short, so
holding the fold \emph{count} constant across history lengths and holding the
fold \emph{size} constant are mutually exclusive, and they answer different
questions.}
